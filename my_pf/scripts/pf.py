...
class ParticleFilter:
	...
	def publish_a_particle(self,msg):
		particle = msg
		self.a_particle_pub.publish(PoseStamped(header=Header(stamp=rospy.Time.now(),frame_id=self.map_frame),pose = particle))
		#Pose(postion = Point(x=4))
	...
	def scan_received(self, msg):
		...
		self.publish_a_particle(self.robot_pose) #jasper
